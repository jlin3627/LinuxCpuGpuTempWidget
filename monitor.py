...
def get_all_metrics() -> Dict[str, Optional[float]]:
    """
    Fetches system temperatures, power metrics, RAM, and VRAM usage.
    """
    metrics = {CPU_TCTL: None, CPU_TCCD1: None, GPU_TEMP: None, GPU_POWER: None, GPU_VRAM: None, RAM_USAGE: None}
...
def _get_gpu_info() -> Dict[str, Optional[float]]:
    ...
    info = {'temp': None, 'power': None, 'vram': None}
    try:
        # Querying temperature, power draw, used memory, and total memory
        res = subprocess.run(
        ...
        if len(parts) >= 4:
            ...
            used_vram = float(parts[2].strip()) / 1024.0 # Convert MiB to GiB
            total_vram = float(parts[3].strip()) / 1024.0 # Convert MiB to GiB
            info['vram'] = {
